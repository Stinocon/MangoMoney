...
/**
 * Valida coerenza tra Overview e Statistics tabs
 */
export const validateTabConsistency = (
  overviewTotals: any, 
  statsTotals: any, 
  ...
  const fields = ['netWorth', 'totalAssets', 'totalDebts'];
  
  fields.forEach(field => {
    const overviewVal = overviewTotals[field] || 0;
    const statsVal = statsTotals[field] || 0;
    const diff = Math.abs(overviewVal - statsVal);
    const relDiff = overviewVal > 0 ? diff / overviewVal : 0;
    
    if (relDiff > tolerance) {
      warnings.push(
        `${field}: Overview=€${overviewVal.toFixed(2)}, Stats=€${statsVal.toFixed(2)} (diff: ${(relDiff*100).toFixed(1)}%)`
      );
    }
  });
...
/**
 * Valida reconciliation posizioni globali vs individuali
 */
export const validatePositionsReconciliation = (
  globalPositions: any[],
  individualPositions: any[], 
  tolerance: number = 0.05
): string[] => {
  ...
  globalPositions.forEach(global => {
    const linked = individualPositions.filter(ind => ind.linkedToGlobal === global.id);
    const globalVal = global.amount || 0;
    const individualTotal = linked.reduce((sum, ind) => 
      sum + ((ind.quantity || 0) * (ind.currentPrice || 0)), 0
    );
    
    const diff = Math.abs(globalVal - individualTotal);
    const relDiff = globalVal > 0 ? diff / globalVal : 0;
    
    if (relDiff > tolerance && diff > 10) {
      warnings.push(
        `"${global.name}": Globale=€${globalVal.toFixed(0)}, Individuali=€${individualTotal.toFixed(0)} (${(relDiff*100).toFixed(1)}%)`
      );
    }
  });
